...
// "Scope Failure" modifications by Evgeny Panasyuk

#ifndef FILE_boost_scope_failure_hpp_INCLUDED
#define FILE_boost_scope_failure_hpp_INCLUDED
...
// WARNING: BOOST_SCOPE_FAILURE_AUX_GCC also used by some regression test.
#if defined(__GNUC__) && !defined(BOOST_INTEL)
#   define BOOST_SCOPE_FAILURE_AUX_GCC (__GNUC__ * 100 + __GNUC_MINOR__)
#else
#   define BOOST_SCOPE_FAILURE_AUX_GCC 0
#endif

#if BOOST_WORKAROUND(BOOST_SCOPE_FAILURE_AUX_GCC, BOOST_TESTED_AT(413))
#   define BOOST_SCOPE_FAILURE_AUX_TPL_GCC_WORKAROUND_01 1
#else
#   define BOOST_SCOPE_FAILURE_AUX_TPL_GCC_WORKAROUND_01 0
#endif

#if BOOST_MSVC
#   define BOOST_SCOPE_FAILURE_AUX_TYPEOF_THIS_MSVC_WORKAROUND_01 1
#else
#   define BOOST_SCOPE_FAILURE_AUX_TYPEOF_THIS_MSVC_WORKAROUND_01 0
#endif

// Steven Watanabe's trick with a modification suggested by Kim Barrett
namespace boost { namespace scope_failure { namespace detail {

// Type of a local BOOST_SCOPE_FAILURE_AUX_ARGS variable.
// First use in a local scope will declare the BOOST_SCOPE_FAILURE_AUX_ARGS
// variable, subsequent uses will be resolved as two comparisons
// (cmp1 with 0 and cmp2 with BOOST_SCOPE_FAILURE_AUX_ARGS).
...
{
    T& value;
#if !BOOST_SCOPE_FAILURE_AUX_TPL_GCC_WORKAROUND_01
...
BOOST_TYPEOF_REGISTER_TEMPLATE(boost::scope_failure::detail::wrapper, 1)

#define BOOST_SCOPE_FAILURE_AUX_ARGS boost_scope_failure_aux_args
extern boost::scope_failure::detail::undeclared BOOST_SCOPE_FAILURE_AUX_ARGS;

#define BOOST_SCOPE_FAILURE_AUX_GUARD(id)   \
    BOOST_PP_CAT(boost_se_guard_, id)

#define BOOST_SCOPE_FAILURE_AUX_GUARD_T(id) \
    BOOST_PP_CAT(boost_se_guard_t_, id)

#define BOOST_SCOPE_FAILURE_AUX_PARAMS(id)  \
    BOOST_PP_CAT(boost_se_params_, id)

#define BOOST_SCOPE_FAILURE_AUX_THIS_T(id)  \
    BOOST_PP_CAT(boost_se_this_t_,  id)

#define BOOST_SCOPE_FAILURE_AUX_THIS_CAPTURE_T(id) \
    BOOST_PP_CAT(boost_se_this_capture_t_,  id)

#define BOOST_SCOPE_FAILURE_DETAIL_PARAMS_T(id) \
    BOOST_PP_CAT(boost_se_params_t_, id)

#define BOOST_SCOPE_FAILURE_DETAIL_TAG(id, i) \
    BOOST_PP_SEQ_CAT( (boost_se_tag_)(i)(_)(id) )

#define BOOST_SCOPE_FAILURE_DETAIL_PARAM_THIS(id) \
    BOOST_PP_SEQ_CAT( (boost_se_param_this_)(id) )

#define BOOST_SCOPE_FAILURE_DETAIL_PARAM(id, i, var) \
    BOOST_PP_SEQ_CAT( (boost_se_param_)(i)(_)(id) )

#define BOOST_SCOPE_FAILURE_DETAIL_PARAM_T(id, i, var) \
    BOOST_PP_SEQ_CAT( (boost_se_param_t_)(i)(_)(id) )

#define BOOST_SCOPE_FAILURE_DETAIL_CAPTURE_T(id, i, var) \
    BOOST_PP_SEQ_CAT( (boost_se_capture_t_)(i)(_)(id) )

#define BOOST_SCOPE_FAILURE_AUX_WRAPPED(id, i) \
    BOOST_PP_SEQ_CAT( (boost_se_wrapped_t_)(i)(_)(id) )

#define BOOST_SCOPE_FAILURE_AUX_DEREF(id, i, var) \
    ::boost::scope_failure::detail::deref(var, \
            static_cast<BOOST_SCOPE_FAILURE_DETAIL_TAG(id, i)>(0))

#define BOOST_SCOPE_FAILURE_AUX_MEMBER(r, id, i, var) \
    ::boost::scope_failure::detail::member< \
        BOOST_SCOPE_FAILURE_DETAIL_PARAM_T(id, i, var), \
        BOOST_SCOPE_FAILURE_DETAIL_TAG(id, i) \
    > BOOST_SCOPE_FAILURE_DETAIL_PARAM(id, i, var);

#define BOOST_SCOPE_FAILURE_AUX_ARG_DECL(r, id_ty, i, var) \
    BOOST_PP_COMMA_IF(i) \
    BOOST_PP_TUPLE_ELEM(2, 1, id_ty) \
    BOOST_SCOPE_FAILURE_DETAIL_PARAMS_T(BOOST_PP_TUPLE_ELEM(2, 0, id_ty)):: \
            BOOST_SCOPE_FAILURE_DETAIL_PARAM_T(BOOST_PP_TUPLE_ELEM(2, 0, id_ty), \
                    i, var) \
    var
 
#define BOOST_SCOPE_FAILURE_AUX_ARG(r, id, i, var) \
    BOOST_PP_COMMA_IF(i) \
    boost_se_params_->BOOST_SCOPE_FAILURE_DETAIL_PARAM(id, i, var).value

#define BOOST_SCOPE_FAILURE_DETAIL_TAG_DECL(r, id, i, var) \
    typedef void (*BOOST_SCOPE_FAILURE_DETAIL_TAG(id, i))(int var);
...
// cannot be a `typeof(...)`.
#if BOOST_SCOPE_FAILURE_AUX_TYPEOF_THIS_MSVC_WORKAROUND_01
...
namespace boost { namespace scope_failure { namespace aux {
...
#define BOOST_SCOPE_FAILURE_AUX_TYPEDEF_TYPEOF_THIS_INDEX_(id) \
    BOOST_PP_CAT(boost_se_thistype_index_, id)

#define BOOST_SCOPE_FAILURE_DETAIL_TYPEDEF_TYPEOF_THIS(id, ty, new_type) \
    ...
    enum { \
        BOOST_SCOPE_FAILURE_AUX_TYPEDEF_TYPEOF_THIS_INDEX_(id) = sizeof( \
            *::boost::scope_failure::aux::msvc_typeof_this::encode_start(this)) \
    }; \
    typedef \
        ty ::boost::scope_failure::aux::msvc_typeof_this::msvc_typeid_wrapper< \
            BOOST_SCOPE_FAILURE_AUX_TYPEDEF_TYPEOF_THIS_INDEX_(id) \
        >::type \
        new_type \
    ;
...
#define BOOST_SCOPE_FAILURE_AUX_PARAMS_T_CTOR(id, ty, captures, has_this) \
    /* expand to nothing */

#define BOOST_SCOPE_FAILURE_DETAIL_PARAM_INIT(r, id, i, var) \
    BOOST_PP_COMMA_IF(i) { BOOST_SCOPE_FAILURE_AUX_DEREF(id, i, var) }

#define BOOST_SCOPE_FAILURE_AUX_PARAMS_INIT(id, captures, has_this) \
    BOOST_PP_EXPR_IIF(BOOST_PP_BITOR(has_this, \
            BOOST_PP_LIST_IS_CONS(captures)), \
        = { \
    ) \
    BOOST_PP_LIST_FOR_EACH_I(BOOST_SCOPE_FAILURE_DETAIL_PARAM_INIT, id, captures) \
...
#define BOOST_SCOPE_FAILURE_AUX_CTOR_ARG(r, id, i, var) \
    BOOST_PP_COMMA_IF(i) \
    BOOST_SCOPE_FAILURE_DETAIL_PARAM_T(id, i, var) & BOOST_PP_CAT(a, i)

#define BOOST_SCOPE_FAILURE_AUX_MEMBER_INIT(r, id, i, var) \
    BOOST_PP_COMMA_IF(i) \
    BOOST_SCOPE_FAILURE_DETAIL_PARAM(id, i, var) ( BOOST_PP_CAT(a, i) )

#define BOOST_SCOPE_FAILURE_AUX_CTOR_ARG_THIS_NAME(id) \
    BOOST_PP_CAT(boost_se_this_arg_, id)

#define BOOST_SCOPE_FAILURE_AUX_CTOR_ARG_THIS(id, ty, comma01) \
    BOOST_PP_COMMA_IF(comma01) \
    ty BOOST_SCOPE_FAILURE_DETAIL_PARAMS_T(id)::BOOST_SCOPE_FAILURE_AUX_THIS_T(id) \
            BOOST_SCOPE_FAILURE_AUX_CTOR_ARG_THIS_NAME(id) /* ptr so no & */

#define BOOST_SCOPE_FAILURE_AUX_MEMBER_THIS_INIT(id, comma01) \
    BOOST_PP_COMMA_IF(comma01) \
    BOOST_SCOPE_FAILURE_DETAIL_PARAM_THIS(id)( \
            BOOST_SCOPE_FAILURE_AUX_CTOR_ARG_THIS_NAME(id))

#define BOOST_SCOPE_FAILURE_AUX_PARAMS_T_CTOR(id, ty, captures, has_this) \
    BOOST_SCOPE_FAILURE_DETAIL_PARAMS_T(id)( \
        BOOST_PP_LIST_FOR_EACH_I(BOOST_SCOPE_FAILURE_AUX_CTOR_ARG, id, captures) \
        BOOST_PP_IIF(has_this, \
            BOOST_SCOPE_FAILURE_AUX_CTOR_ARG_THIS \
        , \
        ...
        BOOST_PP_LIST_FOR_EACH_I(BOOST_SCOPE_FAILURE_AUX_MEMBER_INIT, id, \
                captures) \
        BOOST_PP_IIF(has_this, \
            BOOST_SCOPE_FAILURE_AUX_MEMBER_THIS_INIT \
        , \
...
#define BOOST_SCOPE_FAILURE_DETAIL_CAPTURE_DECL(r, id_ty, i, var) \
    struct BOOST_SCOPE_FAILURE_AUX_WRAPPED(BOOST_PP_TUPLE_ELEM(2, 0, id_ty), i) \
        /* no need to use TYPEOF_TPL here because it's within inheritance */ \
        : BOOST_TYPEOF(::boost::scope_failure::detail::wrap( \
                BOOST_SCOPE_FAILURE_AUX_DEREF(BOOST_PP_TUPLE_ELEM(2, 0, id_ty), \
                        i, var))) \
    {}; \
    typedef BOOST_PP_TUPLE_ELEM(2, 1, id_ty) \
        BOOST_SCOPE_FAILURE_AUX_WRAPPED(BOOST_PP_TUPLE_ELEM(2, 0, id_ty), i)::type\
        BOOST_SCOPE_FAILURE_DETAIL_CAPTURE_T(BOOST_PP_TUPLE_ELEM(2, 0, id_ty), \
                i, var) \
    ;
...
#define BOOST_SCOPE_FAILURE_DETAIL_CAPTURE_DECL(r, id_ty, i, var) \
    typedef \
        ...
        BOOST_TYPEOF_KEYWORD(BOOST_SCOPE_FAILURE_AUX_DEREF( \
...
#define BOOST_SCOPE_FAILURE_DETAIL_CAPTURE_DECL(r, id_ty, i, var) \
    typedef \
        ...
        BOOST_TYPEOF(::boost::scope_failure::detail::wrap( \
                BOOST_SCOPE_FAILURE_AUX_DEREF(BOOST_PP_TUPLE_ELEM(2, 0, id_ty), \
                        i, var))) \
        BOOST_SCOPE_FAILURE_AUX_WRAPPED(BOOST_PP_TUPLE_ELEM(2, 0, id_ty), i) \
    ; \
    typedef BOOST_PP_TUPLE_ELEM(2, 1, id_ty) \
        BOOST_SCOPE_FAILURE_AUX_WRAPPED(BOOST_PP_TUPLE_ELEM(2, 0, id_ty), i)::type\
        BOOST_SCOPE_FAILURE_DETAIL_CAPTURE_T(BOOST_PP_TUPLE_ELEM(2, 0, id_ty), \
...
#define BOOST_SCOPE_FAILURE_DETAIL_PARAM_DECL(r, id_ty, i, var) \
    typedef \
        BOOST_SCOPE_FAILURE_DETAIL_CAPTURE_T(BOOST_PP_TUPLE_ELEM(2, 0, id_ty), \
                i, var) \
        BOOST_SCOPE_FAILURE_DETAIL_PARAM_T(BOOST_PP_TUPLE_ELEM(2, 0, id_ty), \
...
#define BOOST_SCOPE_FAILURE_AUX_TRAITS_OP_CAPTURE(d, captures, this01, capture) \
    (BOOST_PP_LIST_APPEND(captures, (capture, BOOST_PP_NIL)), this01)

#define BOOST_SCOPE_FAILURE_AUX_TRAITS_OP_THIS(d, captures, this01, this_) \
    (captures, 1 /* has this (note, no error if multiple this_) */)

#define BOOST_SCOPE_FAILURE_AUX_TRAITS_OP(d, captures_this, capture) \
    BOOST_PP_IIF(BOOST_LOCAL_FUNCTION_DETAIL_PP_KEYWORD_IS_THISUNDERSCORE_BACK(\
            capture), \
        BOOST_SCOPE_FAILURE_AUX_TRAITS_OP_THIS \
    , \
        BOOST_SCOPE_FAILURE_AUX_TRAITS_OP_CAPTURE \
    )(d, BOOST_PP_TUPLE_ELEM(2, 0, captures_this), \
            BOOST_PP_TUPLE_ELEM(2, 1, captures_this), capture)

// ref_val: & | =
#define BOOST_SCOPE_FAILURE_AUX_TRAITS_ALL_OP(ref_val, traits) \
    ( \
        BOOST_PP_LIST_APPEND((ref_val, BOOST_PP_NIL), \
                BOOST_SCOPE_FAILURE_AUX_TRAITS_CAPTURES(traits)) \
    , \
        BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits) \
    )

#define BOOST_SCOPE_FAILURE_AUX_TRAITS(captures) \
    BOOST_PP_LIST_FOLD_LEFT(BOOST_SCOPE_FAILURE_AUX_TRAITS_OP, \
            (BOOST_PP_NIL, 0), captures)

#define BOOST_SCOPE_FAILURE_AUX_TRAITS_ALL(captures) \
    BOOST_SCOPE_FAILURE_AUX_TRAITS_ALL_OP(BOOST_PP_LIST_FIRST(captures), \
            BOOST_SCOPE_FAILURE_AUX_TRAITS(BOOST_PP_LIST_REST(captures)))

#define BOOST_SCOPE_FAILURE_AUX_TRAITS_CAPTURES(traits) \
    BOOST_PP_TUPLE_ELEM(2, 0, traits)

#define BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits) \
...
#define BOOST_SCOPE_FAILURE_AUX_LAMBDA_PARAMS(id) \
    BOOST_PP_CAT(boost_se_lambda_params_, id)

#define BOOST_SCOPE_FAILURE_AUX_LAMBDA_THIS_CAPTURE_TYPE(id) \
    BOOST_PP_CAT(boost_se_lambda_this_t_, id)

#define BOOST_SCOPE_FAILURE_AUX_LAMBDA_THIS_PARAM_TYPE(id) \
    BOOST_PP_CAT(boost_se_lambda_this_capture_t_, id)

#define BOOST_SCOPE_FAILURE_AUX_LAMBDA_THIS_TYPE(id, ty) \
    ty BOOST_SCOPE_FAILURE_AUX_LAMBDA_PARAMS(id):: \
            BOOST_SCOPE_FAILURE_AUX_LAMBDA_THIS_PARAM_TYPE(id)

// Precondition: HAS_THIS(traits).
#define BOOST_SCOPE_FAILURE_AUX_LAMBDA_THIS_TYPEDEFS(id, ty, traits) \
    BOOST_SCOPE_FAILURE_DETAIL_TYPEDEF_TYPEOF_THIS(id, ty, \
            BOOST_SCOPE_FAILURE_AUX_LAMBDA_THIS_CAPTURE_TYPE(id)) \
    /* capture type for workaround GCC internal error (even on later C++11) */ \
    struct BOOST_SCOPE_FAILURE_AUX_LAMBDA_PARAMS(id) { \
        typedef BOOST_SCOPE_FAILURE_AUX_LAMBDA_THIS_CAPTURE_TYPE(id) \
                BOOST_SCOPE_FAILURE_AUX_LAMBDA_THIS_PARAM_TYPE(id); \
    };

#define BOOST_SCOPE_FAILURE_AUX_IMPL_LAMBDA(id, ty, traits) \
    BOOST_PP_IIF(BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits), \
        /* no need for TYPEDEF THIS MSVC workaround on C++11 */ \
        BOOST_SCOPE_FAILURE_AUX_LAMBDA_THIS_TYPEDEFS \
    , \
        BOOST_PP_TUPLE_EAT(3) \
    )(id, ty, traits) \
    ::boost::scope_failure::aux::guard< \
        BOOST_PP_IIF(BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits), \
            BOOST_SCOPE_FAILURE_AUX_LAMBDA_THIS_TYPE \
        , \
            BOOST_PP_TUPLE_EAT(2) \
        )(id, ty) \
    > BOOST_SCOPE_FAILURE_AUX_GUARD(id) \
        BOOST_PP_EXPR_IIF(BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits), \
            (this) \
        ) \
    ; \
    BOOST_SCOPE_FAILURE_AUX_GUARD(id) = [ \
        BOOST_PP_LIST_ENUM(BOOST_SCOPE_FAILURE_AUX_TRAITS_CAPTURES(traits)) \
    ]( \
        BOOST_PP_IIF(BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits), \
            BOOST_SCOPE_FAILURE_AUX_LAMBDA_THIS_TYPE \
        , \
        ...
        BOOST_PP_EXPR_IIF(BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits), this_) \
    ) mutable /* can change value captures (as with SCOPE_FAILURE) */ -> void

#endif // Lambdas.

#if defined(BOOST_SCOPE_FAILURE_CONFIG_USE_LAMBDAS) && \
        !defined(BOOST_NO_LAMBDAS) // Use lambda for SCOPE_FAILURE (not just _ALL).

#define BOOST_SCOPE_FAILURE_AUX_IMPL(id, ty, traits) \
    BOOST_SCOPE_FAILURE_AUX_IMPL_LAMBDA(id, ty, traits)

#else // Not using lambdas.

// ty: EMPTY() | typename
#define BOOST_SCOPE_FAILURE_AUX_IMPL(id, ty, traits) \
    BOOST_PP_LIST_FOR_EACH_I(BOOST_SCOPE_FAILURE_DETAIL_TAG_DECL, id, \
            BOOST_SCOPE_FAILURE_AUX_TRAITS_CAPTURES(traits)) \
    BOOST_PP_LIST_FOR_EACH_I(BOOST_SCOPE_FAILURE_DETAIL_CAPTURE_DECL, (id, ty), \
            BOOST_SCOPE_FAILURE_AUX_TRAITS_CAPTURES(traits)) \
    BOOST_PP_IIF(BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits), \
        BOOST_SCOPE_FAILURE_DETAIL_TYPEDEF_TYPEOF_THIS \
    , \
        BOOST_PP_TUPLE_EAT(3) \
    )(id, ty, BOOST_SCOPE_FAILURE_AUX_THIS_CAPTURE_T(id)) \
    struct BOOST_SCOPE_FAILURE_DETAIL_PARAMS_T(id) { \
        /* interim capture types to workaround internal errors on old GCC */ \
        BOOST_PP_LIST_FOR_EACH_I(BOOST_SCOPE_FAILURE_DETAIL_PARAM_DECL, (id, ty), \
                BOOST_SCOPE_FAILURE_AUX_TRAITS_CAPTURES(traits)) \
        BOOST_PP_EXPR_IIF(BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits), \
            typedef BOOST_SCOPE_FAILURE_AUX_THIS_CAPTURE_T(id) \
                    BOOST_SCOPE_FAILURE_AUX_THIS_T(id) ; \
        ) \
        BOOST_PP_LIST_FOR_EACH_I(BOOST_SCOPE_FAILURE_AUX_MEMBER, id, \
                BOOST_SCOPE_FAILURE_AUX_TRAITS_CAPTURES(traits)) \
        BOOST_PP_EXPR_IIF(BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits), \
            BOOST_SCOPE_FAILURE_AUX_THIS_T(id) \
                    BOOST_SCOPE_FAILURE_DETAIL_PARAM_THIS(id) ; \
        ) \
        BOOST_SCOPE_FAILURE_AUX_PARAMS_T_CTOR(id, ty, \
                BOOST_SCOPE_FAILURE_AUX_TRAITS_CAPTURES(traits), \
                BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits)) \
    } BOOST_SCOPE_FAILURE_AUX_PARAMS(id) \
        BOOST_SCOPE_FAILURE_AUX_PARAMS_INIT(id, \
                BOOST_SCOPE_FAILURE_AUX_TRAITS_CAPTURES(traits), \
                BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits)) \
    ; \
    ::boost::scope_failure::detail::declared< \
        ::boost::scope_failure::detail::resolve< \
            sizeof(BOOST_SCOPE_FAILURE_AUX_ARGS) \
        >::cmp1<0>::cmp2 \
    > BOOST_SCOPE_FAILURE_AUX_ARGS; \
    BOOST_SCOPE_FAILURE_AUX_ARGS.value = &BOOST_SCOPE_FAILURE_AUX_PARAMS(id); \
    struct BOOST_SCOPE_FAILURE_AUX_GUARD_T(id) { \
        BOOST_SCOPE_FAILURE_DETAIL_PARAMS_T(id)* boost_se_params_; \
        BOOST_SCOPE_FAILURE_AUX_GUARD_T(id) (void* boost_se_params) \
            : boost_se_params_( \
                    (BOOST_SCOPE_FAILURE_DETAIL_PARAMS_T(id)*)boost_se_params) \
        {} \
        private : stack_unwinding::unwinding_indicator indicator; \
        public : \
        ~BOOST_SCOPE_FAILURE_AUX_GUARD_T(id)() { \
            if(indicator.unwinding()) boost_se_body( \
                BOOST_PP_LIST_FOR_EACH_I(BOOST_SCOPE_FAILURE_AUX_ARG, id, \
                        BOOST_SCOPE_FAILURE_AUX_TRAITS_CAPTURES(traits)) \
                BOOST_PP_COMMA_IF(BOOST_PP_BITAND(BOOST_PP_LIST_IS_CONS( \
                        BOOST_SCOPE_FAILURE_AUX_TRAITS_CAPTURES(traits)), \
                        BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits))) \
                BOOST_PP_EXPR_IIF(BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS( \
                        traits), \
                    boost_se_params_->BOOST_SCOPE_FAILURE_DETAIL_PARAM_THIS(id) \
                ) \
            ); \
        } \
        static void boost_se_body( \
            BOOST_PP_LIST_FOR_EACH_I(BOOST_SCOPE_FAILURE_AUX_ARG_DECL, (id, ty), \
                    BOOST_SCOPE_FAILURE_AUX_TRAITS_CAPTURES(traits)) \
            BOOST_PP_COMMA_IF(BOOST_PP_BITAND(BOOST_PP_LIST_IS_CONS( \
                    BOOST_SCOPE_FAILURE_AUX_TRAITS_CAPTURES(traits)), \
                    BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits))) \
            BOOST_PP_EXPR_IIF(BOOST_SCOPE_FAILURE_AUX_TRAITS_HAS_THIS(traits), \
                ty BOOST_SCOPE_FAILURE_DETAIL_PARAMS_T(id):: \
                        BOOST_SCOPE_FAILURE_AUX_THIS_T(id) this_ \
            ) \
        )
...
#   define BOOST_SCOPE_FAILURE_ID(id, void_or_seq) \
        BOOST_SCOPE_FAILURE_AUX_IMPL(id, BOOST_PP_EMPTY(), \
                BOOST_SCOPE_FAILURE_AUX_TRAITS( \
                        BOOST_LOCAL_FUNCTION_DETAIL_PP_VOID_LIST(void_or_seq)))
#   define BOOST_SCOPE_FAILURE_ID_TPL(id, void_or_seq) \
        BOOST_SCOPE_FAILURE_AUX_IMPL(id, typename, \
                BOOST_SCOPE_FAILURE_AUX_TRAITS( \
                        BOOST_LOCAL_FUNCTION_DETAIL_PP_VOID_LIST(void_or_seq)))
#   define BOOST_SCOPE_FAILURE(void_or_seq) \
        BOOST_SCOPE_FAILURE_ID(BOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER, \
                void_or_seq)
#   define BOOST_SCOPE_FAILURE_TPL(void_or_seq) \
        BOOST_SCOPE_FAILURE_ID_TPL(BOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER, \
                void_or_seq)
#   if !defined(BOOST_NO_LAMBDAS)
#       define BOOST_SCOPE_FAILURE_ALL_ID(id, seq) \
            BOOST_SCOPE_FAILURE_AUX_IMPL_LAMBDA(id, \
                    ...
                    typename, \
                    BOOST_SCOPE_FAILURE_AUX_TRAITS_ALL( \
                            BOOST_LOCAL_FUNCTION_DETAIL_PP_NON_VOID_LIST(seq)))
#       define BOOST_SCOPE_FAILURE_ALL(seq) \
            BOOST_SCOPE_FAILURE_ALL_ID( \
                    BOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER, seq)
#   endif
#else // Variadic macros (both sequences and variadic tuples).
#   define BOOST_SCOPE_FAILURE_ID(id, ...) \
        BOOST_SCOPE_FAILURE_AUX_IMPL(id, BOOST_PP_EMPTY(), \
                BOOST_SCOPE_FAILURE_AUX_TRAITS( \
                        BOOST_LOCAL_FUNCTION_DETAIL_PP_VOID_LIST(__VA_ARGS__)))
#   define BOOST_SCOPE_FAILURE_ID_TPL(id, ...) \
        BOOST_SCOPE_FAILURE_AUX_IMPL(id, typename, \
                BOOST_SCOPE_FAILURE_AUX_TRAITS( \
                        BOOST_LOCAL_FUNCTION_DETAIL_PP_VOID_LIST(__VA_ARGS__)))
#   define BOOST_SCOPE_FAILURE(...) \
        BOOST_SCOPE_FAILURE_ID(BOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER, \
                __VA_ARGS__)
#   define BOOST_SCOPE_FAILURE_TPL(...) \
        BOOST_SCOPE_FAILURE_ID_TPL(BOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER, \
                __VA_ARGS__)
#   if !defined(BOOST_NO_LAMBDAS)
#       define BOOST_SCOPE_FAILURE_ALL_ID(id, ...) \
            BOOST_SCOPE_FAILURE_AUX_IMPL_LAMBDA(id, \
                    /* C++11 allows to use typename outside templates so */ \
                    /* always typename here and no need for ..._ALL_TPL */ \
                    /* (if a C++11 compiler does not implement this use of */ \
                    ...
                    BOOST_SCOPE_FAILURE_AUX_TRAITS_ALL( \
                            BOOST_LOCAL_FUNCTION_DETAIL_PP_NON_VOID_LIST( \
                                    __VA_ARGS__)))
#       define BOOST_SCOPE_FAILURE_ALL(...) \
            BOOST_SCOPE_FAILURE_ALL_ID( \
...
#if defined(BOOST_SCOPE_FAILURE_CONFIG_USE_LAMBDAS) && \
        !defined(BOOST_NO_LAMBDAS) // Use lambdas for SCOPE_FAILURE (not just ALL).
#   define BOOST_SCOPE_FAILURE_END_ID(id) \
        ; /* lambdas ended with just `;` */
#else // Not using lambdas.
#   define BOOST_SCOPE_FAILURE_END_ID(id) \
        } BOOST_SCOPE_FAILURE_AUX_GUARD(id)(BOOST_SCOPE_FAILURE_AUX_ARGS.value);
#endif // Using lambdas.
#define BOOST_SCOPE_FAILURE_END \
    BOOST_SCOPE_FAILURE_END_ID(BOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER)
...
@code
    { // Some local scope.
        ...
        BOOST_SCOPE_FAILURE(capture_list) {
            ... // Body code.
...
@RefMacro{BOOST_SCOPE_FAILURE_END}.
...
@endcode
Furthermore\, if @RefMacro{BOOST_SCOPE_FAILURE_CONFIG_USE_LAMBDAS} is defined on
...
@RefMacro{BOOST_SCOPE_FAILURE_END} and <c>this</c> can be used instead of
<c>this_</c>:
@code
    capture:
            [&]variable | this_ | this
...
However, on all compilers the same macro @RefMacro{BOOST_SCOPE_FAILURE} also
...
On various GCC versions the special macro @RefMacro{BOOST_SCOPE_FAILURE_TPL} must
be used instead of @RefMacro{BOOST_SCOPE_FAILURE} within templates (to maximize
portability, it is recommended to always use @RefMacro{BOOST_SCOPE_FAILURE_TPL}
within templates).

On C++11, it is possible capture all variables in scope without listing their
names one-by-one using the macro @RefMacro{BOOST_SCOPE_FAILURE_ALL}.

In general, the special macro @RefMacro{BOOST_SCOPE_FAILURE_ID} must be used
instead of @RefMacro{BOOST_SCOPE_FAILURE} when it is necessary to expand multiple
...
@RefMacro{BOOST_SCOPE_FAILURE_TPL}, @RefMacro{BOOST_SCOPE_FAILURE_ALL},
@RefMacro{BOOST_SCOPE_FAILURE_END}, @RefMacro{BOOST_SCOPE_FAILURE_ID}.
*/
#define BOOST_SCOPE_FAILURE(capture_list)

/**
@brief This macro is a workaround for various versions of GCC to declare scope
exits within templates.

Various versions of the GCC compiler do not compile @RefMacro{BOOST_SCOPE_FAILURE}
inside function templates.
As a workaround, @RefMacro{BOOST_SCOPE_FAILURE_TPL} should be used instead of
@RefMacro{BOOST_SCOPE_FAILURE} in these cases:

@code
    { // Some local scope.
        ...
        BOOST_SCOPE_FAILURE_TPL(capture_list) {
            ... // Body code.
        } BOOST_SCOPE_FAILURE_END
        ...
    }
...
The syntax of @RefMacro{BOOST_SCOPE_FAILURE_TPL} is the exact same as the one of
@RefMacro{BOOST_SCOPE_FAILURE} (see @RefMacro{BOOST_SCOPE_FAILURE} for more
information).

On C++11 compilers, @RefMacro{BOOST_SCOPE_FAILURE_TPL} is not needed because
@RefMacro{BOOST_SCOPE_FAILURE} always compiles on GCC versions that support C++11.
However, @RefMacro{BOOST_SCOPE_FAILURE_TPL} is still provided on C++11 so to write code that is portable between C++03 and C++11 compilers.
It is recommended to always use @RefMacro{BOOST_SCOPE_FAILURE_TPL} within
templates so to maximize portability.

In general, the special macro @RefMacro{BOOST_SCOPE_FAILURE_ID_TPL} must be used
instead of @RefMacro{BOOST_SCOPE_FAILURE_TPL} when it is necessary to expand
...
implementation of the @RefMacro{BOOST_SCOPE_FAILURE_TPL} macro).

@Note Although @RefMacro{BOOST_SCOPE_FAILURE_TPL} has the same suffix as
<c>BOOST_TYPEOF_TPL</c>, it does not follow the Boost.Typeof convention.

@See @RefSect{tutorial, Tutorial} section, @RefMacro{BOOST_SCOPE_FAILURE},
@RefMacro{BOOST_SCOPE_FAILURE_END}, @RefMacro{BOOST_SCOPE_FAILURE_ID_TPL}.
*/
#define BOOST_SCOPE_FAILURE_TPL(capture_list)

/**
@brief This macro allows to expand multiple scope exit declarations on the same
line.

This macro is equivalent to @RefMacro{BOOST_SCOPE_FAILURE} but it can be expanded
multiple times on the same line if different identifiers <c>id</c> are provided
for each expansion (see @RefMacro{BOOST_SCOPE_FAILURE} for more information).

@Params
@Param{id,
A unique identifier token which can be concatenated by the preprocessor
(<c>__LINE__</c>\, <c>scope_failure_number_1_on_line_123</c>\, a combination of
alphanumeric tokens\, etc).
}
@Param{capture_list,
Same as the <c>capture_list</c> parameter of the @RefMacro{BOOST_SCOPE_FAILURE}
macro.
}
@EndParams
...
@See @RefSect{tutorial, Tutorial} section, @RefMacro{BOOST_SCOPE_FAILURE},
@RefMacro{BOOST_SCOPE_FAILURE_END_ID}, @RefMacro{BOOST_SCOPE_FAILURE_ALL_ID},
@RefMacro{BOOST_SCOPE_FAILURE_ID_TPL}.
*/
#define BOOST_SCOPE_FAILURE_ID(id, capture_list)
...
This macro is equivalent to @RefMacro{BOOST_SCOPE_FAILURE_TPL} but it can be
expanded multiple times on the same line if different identifiers <c>id</c> are
provided for each expansion (see @RefMacro{BOOST_SCOPE_FAILURE_TPL} for more
information).
As with @RefMacro{BOOST_SCOPE_FAILURE_TPL}, it is recommended to always use this
...
@RefMacro{BOOST_SCOPE_FAILURE_TPL} macro.
}
@EndParams
...
@See @RefSect{tutorial, Tutorial} section, @RefMacro{BOOST_SCOPE_FAILURE_TPL},
@RefMacro{BOOST_SCOPE_FAILURE_END_ID}, @RefMacro{BOOST_SCOPE_FAILURE_ID},
@RefMacro{BOOST_SCOPE_FAILURE_ALL_ID}.
*/
#define BOOST_SCOPE_FAILURE_ID_TPL(id, capture_list)
...
A part from that, this macro works like @RefMacro{BOOST_SCOPE_FAILURE} (see @RefMacro{BOOST_SCOPE_FAILURE} for more information):

@code
    { // Some local scope.
        ...
        BOOST_SCOPE_FAILURE_ALL(capture_list) { // C++11 only.
            ... // Body code.
        }; // Use `;` instead of `BOOST_SCOPE_FAILURE_END` (C++11 only).
        ...
    }
@endcode

Note how the end of the scope exit body declared by this macro must be marked
by a semi-column <c>;</c> (and not by @RefMacro{BOOST_SCOPE_FAILURE_END}).
...
On all compilers, the same macro @RefMacro{BOOST_SCOPE_FAILURE_ALL} also allows to
...
so it can never be <c>void</c> (<c>BOOST_SCOPE_FAILURE(void)</c> should be used
to program scope exits with an empty capture list).

In general, the special macro @RefMacro{BOOST_SCOPE_FAILURE_ALL_ID} must be used
instead of @RefMacro{BOOST_SCOPE_FAILURE_ALL} when it is necessary to expand
multiple scope exit declarations on the same line.
...
@RefMacro{BOOST_SCOPE_FAILURE} captures (unless programmers define the
@RefMacro{BOOST_SCOPE_FAILURE_CONFIG_USE_LAMBDAS} macro).
For example, like C++11 lambda functions, @RefMacro{BOOST_SCOPE_FAILURE_ALL}
requires to capture data members by capturing the object <c>this</c> while
@RefMacro{BOOST_SCOPE_FAILURE} allows to capture data members directly and without
...
for a <c>BOOST_SCOPE_FAILURE_ALL_TPL</c> macro).

@See @RefSect{tutorial, Tutorial} section,
@RefSect{no_variadic_macros, No Variadic Macros} section,
@RefMacro{BOOST_SCOPE_FAILURE}, @RefMacro{BOOST_SCOPE_FAILURE_ALL_ID}.
*/
#define BOOST_SCOPE_FAILURE_ALL(capture_list)

/**
@brief This macro allows to expand on the same line multiple scope exits that
capture all variables in scope (C++11 only).

This macro is equivalent to @RefMacro{BOOST_SCOPE_FAILURE_ALL} but it can be
expanded multiple times on the same line if different identifiers <c>id</c> are
provided for each expansion (see @RefMacro{BOOST_SCOPE_FAILURE_ALL} for more
information).
As with @RefMacro{BOOST_SCOPE_FAILURE_ALL}, this macro is only available on C++11
...
@Param{capture_list,
Same as the <c>capture_list</c> parameter of the
@RefMacro{BOOST_SCOPE_FAILURE_ALL} macro.
}
@EndParams
...
@See @RefSect{tutorial, Tutorial} section, @RefMacro{BOOST_SCOPE_FAILURE_ALL},
@RefMacro{BOOST_SCOPE_FAILURE_ID}.
*/
#define BOOST_SCOPE_FAILURE_ALL_ID(id, capture_list)
...
either @RefMacro{BOOST_SCOPE_FAILURE} or @RefMacro{BOOST_SCOPE_FAILURE_TPL}:
...
In general, the special macro @RefMacro{BOOST_SCOPE_FAILURE_END_ID} must be used
instead of @RefMacro{BOOST_SCOPE_FAILURE_END} when it is necessary to expand
multiple scope exit bodies on the same line.

@Note If programmers define the @RefMacro{BOOST_SCOPE_FAILURE_CONFIG_USE_LAMBDAS}
...
@See @RefSect{tutorial, Tutorial} section, @RefMacro{BOOST_SCOPE_FAILURE},
@RefMacro{BOOST_SCOPE_FAILURE_TPL}, @RefMacro{BOOST_SCOPE_FAILURE_END_ID}.
*/
#define BOOST_SCOPE_FAILURE_END
...
This macro is equivalent to @RefMacro{BOOST_SCOPE_FAILURE_END} but it can be
expanded multiple times on the same line if different identifiers <c>id</c> are
provided for each expansion (see @RefMacro{BOOST_SCOPE_FAILURE_END} for more
information).
...
@See @RefMacro{BOOST_SCOPE_FAILURE_ID}, @RefMacro{BOOST_SCOPE_FAILURE_ID_TPL},
@RefMacro{BOOST_SCOPE_FAILURE_END}.
*/
#define BOOST_SCOPE_FAILURE_END_ID(id)
...
@RefMacro{BOOST_SCOPE_FAILURE} and @RefMacro{BOOST_SCOPE_FAILURE_TPL} macros will use
...
@RefMacro{BOOST_SCOPE_FAILURE} and @RefMacro{BOOST_SCOPE_FAILURE_TPL} are no longer
backward compatible (and this is why this macro is not defined by default).

A semicolon <c>;</c> can be used instead of @RefMacro{BOOST_SCOPE_FAILURE_END}
when this configuration macro is defined (but it is recommended to always use
@RefMacro{BOOST_SCOPE_FAILURE_END} so to maximize portability).

@Note This configuration macro does not control the definition of
@RefMacro{BOOST_SCOPE_FAILURE_ALL} which is always and automatically defined on
compilers that support C++11 lambda functions.

@See @RefMacro{BOOST_SCOPE_FAILURE}, @RefMacro{BOOST_SCOPE_FAILURE_TPL},
@RefMacro{BOOST_SCOPE_FAILURE_END}.
*/
#define BOOST_SCOPE_FAILURE_CONFIG_USE_LAMBDAS

#endif // DOXYGEN

#endif // #ifndef FILE_boost_scope_failure_hpp_INCLUDED
